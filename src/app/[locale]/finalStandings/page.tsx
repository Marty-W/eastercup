"use client";
import { TEAM_CATEGORIRES, FINAL_STANDINGS } from "@/lib/conts";
...
import { FinalStandingsWrapper } from "@/components/finalStandingsWrapper";

export default function FinalResults() {
  const [selectedCategory, setSelectedCategory] =
    useState<TeamCategory>("U11 MIX");

  const selectedCategoryData = FINAL_STANDINGS[selectedCategory];
  return (
        ...
        {TEAM_CATEGORIRES.map((category) => {
          return (
            <TabsContent value={category} key={category}>
              <FinalStandingsWrapper categoryResult={selectedCategoryData} />
            </TabsContent>
          );
        })}
      </Tabs>
